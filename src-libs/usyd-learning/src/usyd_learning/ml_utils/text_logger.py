...
class TextLogger:
    ...
    def __init__(self, file_extension: str = ".log"):
        """
        Create text logger with log name and store path(defalt is './log')
        """

        self._log_name = None       # Name of logn file
        self._log_path = None       # store log file to path
    ...
    def open(self, log_name: str = "text_log", path: str = "./log"):
        ...
        if String.is_none_or_empty(log_name):
            raise exception("Log name can't be empty.")

        self._log_name = log_name       # Name of logn file

...
class KeyValueMap:
    ...
    def get(self, key: str, default_value: any = None):
        ...
        for k in key.split("|"):
            ...
            if k in self._keys_map:
                return self.__map_get(self, k, default_value)
    ...
    @staticmethod
    def build_key_map(from_dict: dict, start_key_path = "/"):
    ...
    @staticmethod
    def __build_one_key_map(start_dict: dict, start_key_path, up_key_name):

        def __get_key_name(up_key_name, name):
            if String.is_none_or_empty(up_key_name) or up_key_name == "/":
                return name
            else:
                return f"{up_key_name}{KeyValueMap.__CONST_SPLIT_CHAR}{name}"

        m = {}
        if isinstance(start_dict, list):
            for index in range(len(start_dict)):
                key_name = __get_key_name(up_key_name, index)
                v = KeyValueMap.__build_one_key_map(start_dict[index], f"{start_key_path}/{index}", key_name)
                m.update(v)
        else:

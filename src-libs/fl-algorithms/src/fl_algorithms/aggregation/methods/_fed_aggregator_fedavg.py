...
class _Aggregator_FedAvg(AbstractFedServerAggregator):
    """
    ...
    def _before_aggregation(self) -> None:
        print(f"[FedAvg] Starting aggregation with {len(self._aggregation_data_list)} clients...")
    ...
    def _do_aggregation(self) -> None:
        ...
        for i, (_, vol) in enumerate(self._aggregation_data_list):
            print(f"  Client {i}: {vol} samples ({vol / total_data_vol * 100:.1f}%)")
        ...
        print(f"[FedAvg] Aggregated first param mean: {new_weights[first_param_name].mean():.6f}")

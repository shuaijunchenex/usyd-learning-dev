...
class ModelEvaluator:
    ...
    def print_results(self):
        ...
        if not self.latest_metrics:
            print("No evaluation metrics available. Please run .evaluate() first.")
            return

        print("Evaluation Summary:")
        print(f"  - Loss     : {self.latest_metrics['average_loss']:.4f}")
        print(f"  - Accuracy : {self.latest_metrics['accuracy'] * 100:.2f}%")
        print(f"  - Precision: {self.latest_metrics['precision']:.4f}")
        print(f"  - Recall   : {self.latest_metrics['recall']:.4f}")
        print(f"  - F1-Score : {self.latest_metrics['f1_score']:.4f}")
        print(f"  - Samples  : {self.latest_metrics['total_test_samples']}")
